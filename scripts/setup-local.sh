...
CONFIG_FILE="config/docker_compose.local.toml"
DASHBOARD_CONFIG_FILE="config/dashboard.local.toml"

API_URL="http://localhost:8080"
DASHBOARD_URL="http://localhost:9000"
SDK_URL="http://localhost:9050"
...
show_banner() {
    printf "${BLUE}${BOLD}\n"
    printf "  Hyperswitch — LOCAL environment (localhost)\n"
    printf "${NC}\n"
}
...
setup_config() {
    ...
    echo "# Local environment setup" >"${env_file}"
...
check_services_health() {
    local HYPERSWITCH_HEALTH_URL="${API_URL}/health"
    local HYPERSWITCH_DEEP_HEALTH_URL="${API_URL}/health/ready"
    ...
    if [ "${is_success}" = true ]; then
        VERSION=$(curl --silent --output /dev/null --request GET --write-out '%header{x-hyperswitch-version}' "${API_URL}" | sed 's/-dirty$//')
...
print_access_info() {
    printf "${BLUE}"
    printf "╔════════════════════════════════════════════════════════════════╗\n"
    printf "║        Hyperswitch — LOCAL environment ready!                  ║\n"
    printf "╚════════════════════════════════════════════════════════════════╝\n"
    printf "${NC}\n"
